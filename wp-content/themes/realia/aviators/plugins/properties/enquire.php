...
add_action('init', 'aviators_enquire_process_form', 9999);

function aviators_enquire_process_form() {

    if ( ! empty( $_POST ) && is_array( $_POST ) && isset( $_POST['enquire_submit'] ) && ! empty($_POST['post_id'] ) ) {
        ...
        if ( ! empty( $agents) || is_array($agents) || !empty($author_email)) {
            $message = '';
