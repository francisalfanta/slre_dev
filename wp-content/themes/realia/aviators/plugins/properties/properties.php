...
$property_metabox = new WPAlchemy_MetaBox(array(
    ...
    'template' => get_template_directory() . '/aviators/plugins/properties/meta.php',
...
function aviators_property_get_metabox() {
    return new Submission_MetaBox(array(
        'id' => '_property_meta',
        'title' => __('Property Options', 'aviators'),
        'template' => get_template_directory() . '/aviators/plugins/properties/meta-submission.php',
        'types' => array('property'),
        'prefix' => '_property_',
        'mode' => WPALCHEMY_MODE_EXTRACT,
    ));
 ...
 */
add_action('wp_enqueue_scripts', '_aviators_property_form_enqueue_js');
...
function aviators_properties_form_generate($metabox) {
    ...
    $property_locations = aviators_dropdown_categories(
        array(
            'selected' => ( is_object( $selectedTerm ) ? $selectedTerm->term_id : $selectedTerm),
    ...
    if ( $selectedTerm ) {
    ...
    } else { //if adding new property, assign sublocations to default location
        $defaultTerm = reset( get_terms( 'locations', array(
            'order'     => 'ASC',
            'order_by'  => 'id',
            'number'    => 1,
            'parent'    => 0,
        ...
        if ( $defaultTerm ) {
            $parentTermId  = $defaultTerm->term_id;
        }
    }
