...
function aviators_core_plugins_list() {
	...
	foreach ( $options->plugins as $plugin ) {
		$plugins[$plugin] = array(
			'path' => get_template_directory() . '/aviators/plugins/' . $plugin,
		);
	}
...
function aviators_core_required_plugins_list() {
	$plugins = array();
	$dirs    = glob( get_template_directory() . '/aviators/core/plugins/*', GLOB_ONLYDIR );

	foreach ( $dirs as $dir ) {
		$parts          = explode( '/', $dir );
		$name           = $parts[count( $parts ) - 1];
		$plugins[$name] = array(
			'path' => get_template_directory() . '/aviators/core/plugins/' . $name,
		);
	}
...
function aviators_core_get_core_plugins_list() {
	return glob( get_template_directory() . '/aviators/core/plugins/*', GLOB_ONLYDIR );
}
...
function aviators_core_require_plugin( $plugin_name ) {
	require_once get_template_directory() . '/aviators/plugins/' . $plugin_name . '/' . $plugin_name . '.php';
}
